...
def test_resolve_unique_raises_on_ambiguous_prefix():
    ...
    with patch.object(SessionInfo, "load", classmethod(lambda cls, ref: SimpleNamespace(first_timestamp="2026-08-20", title="synthetic"))):
...
def test_ambiguous_prefix_resolves_when_one_collision_is_empty():
    ...
    with patch.object(
        SessionInfo,
        "load",
        classmethod(lambda cls, ref: loaded[ref.session_id]),
    ):
...
def test_ambiguous_prefix_still_raises_when_both_collisions_are_real():
    ...
    with patch.object(
        SessionInfo, "load", classmethod(lambda cls, ref: SimpleNamespace(first_timestamp="2026-08-20", title="synthetic"))
    ):
        with pytest.raises(ToolError) as exc:
            resolve_unique_ref_or_none(refs, "aaaaaaaa")
    msg = str(exc.value)
...
def test_discover_dedups_by_session_id(monkeypatch):
    ...
    monkeypatch.setattr(
        claude_provider_mod,
        "load_conversations",
        lambda p: {ID_A: ConversationRef(path=Path(f"/tmp/{ID_A}.jsonl"), worktree=None)},
    )
...
def test_narrow_to_ids_locates_holding_session(monkeypatch):
    ...
    convs = {
        "/a": {ID_A: ConversationRef(path=Path(f"/tmp/{ID_A}.jsonl"), worktree=None)},
        "/b": {ID_B: ConversationRef(path=Path(f"/tmp/{ID_B}.jsonl"), worktree=None)},
    }
