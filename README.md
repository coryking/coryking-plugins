...
A workflow fans out a panel drawn from eight roles — framer (is this even the right question?), skeptic (what's silently missing?), engineer (can this structure represent reality?), auditor (where did every number come from?), calibrator (which way do the estimates lean?), stress-tester (what breaks it?), wildcard (what option did nobody name?), historian (what does the track record say?). Each reviews independently, then they rebut each other in short bounded rounds until nobody has anything left to withdraw, dispute, or update. You can watch the whole argument live, phase by phase, in `/workflows`.

What you get back is a record, not a ruling: the panel's answer to *your question as you asked it*, findings grouped by how the panel left them (seconded / standing / disputed / withdrawn), disagreements presented as disagreements, and every contribution appended verbatim. The panel advises; you decide. Three design commitments keep it honest: your question is the scope boundary and topics you rule out stay out; the record is assembled by a note-taker that never editorializes — the debate's control flow is deterministic script, which can't paraphrase anybody; and the debate terminates — rebuttal rounds allow no new topics, so the disagreement can only shrink, and the loop stops the moment a round produces no live moves, with a hard cap behind it.
