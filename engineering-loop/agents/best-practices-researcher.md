...
2. **Identify Relevant Skills**:
   Match the research topic to available skills semantically — read each `SKILL.md`'s `description` frontmatter and match by intent, not by hardcoded name lookup. The same topic ("Rails best practices", "image generation", "git worktrees") may live under different skill names across installs.
