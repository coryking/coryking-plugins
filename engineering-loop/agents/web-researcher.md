...
- Use `WebSearch` and `WebFetch` for research. If a web tool call fails mid-workflow (rate limit, transport error, blocked URL), narrate the failure briefly and continue with the remaining sources. Do not substitute shell-based fetchers.
- If the caller asked you to write findings to disk, use `Write` for the final report — single-file output, not scattered notes.
