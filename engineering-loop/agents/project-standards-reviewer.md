...
- **Broken cross-references** -- agent names that don't resolve in the current install (typos, references to agents that don't exist, or agents named without the namespace the standards require). Skill-to-skill references using slash syntax inside a SKILL.md where the standards say to use semantic wording. References to tools by platform-specific names without naming the capability class.
