...
**Project-custom reviewers are exempt from the mid-tier override.** In Claude Code, honor the reviewer's frontmatter `model:` without a dispatch-time override. In Codex, read its `codex_model:` (a literal Codex model ID) and pass that as `model` on `spawn_agent`; when the key is absent, use the harness default subject to the user's model constraints. When the spawn tool exposes `fork_turns`, explicitly use `"none"` and supply the review-context bundle: a full-history fork inherits the parent model and cannot accept a model override. The reviewer author owns the cost/quality choice; the built-in mid-tier default does not apply to project-custom personas.
