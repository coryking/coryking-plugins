...
- **Write a run artifact** under `/tmp/engineering-loop/review/<run-id>/` summarizing findings, applied fixes, residual actionable work, and advisory outputs. Orchestrators read this artifact to route residual `downstream-resolver` findings; the skill itself does not file tickets or prompt the user in autofix.
...
- **Never edit files or externalize work.** Do not write `/tmp/engineering-loop/review/<run-id>/`, do not file tickets, and do not commit, push, or create a PR.
...
- **Write a run artifact** under `/tmp/engineering-loop/review/<run-id>/` summarizing findings, applied fixes, and advisory outputs. Include the artifact path in the structured output.
...
The following paths are planning / knowledge-store artifacts and must never be flagged for deletion, removal, or gitignore by any reviewer, even when they look unused from the diff:

- `docs/brainstorms/*` -- requirements documents
- `docs/plans/*.md` -- plan files
- `docs/solutions/*.md` -- solution / learning documents
...
mkdir -p "/tmp/engineering-loop/review/$RUN_ID"
```

Pass `{run_id}` to every persona sub-agent so they can write their full analysis to `/tmp/engineering-loop/review/{run_id}/{reviewer_name}.json`.
...
**Project-custom reviewers are exempt from the mid-tier override.** Do not pass a model override when dispatching a project-custom reviewer — honor whatever its own frontmatter `model:` declares (`inherit` runs it at the session model; an explicit tier runs it there). The built-in mid-tier default exists because each built-in persona's stakes were calibrated when this fork was authored; an unknown project reviewer's stakes cannot be, so the author who wrote it owns the cost/quality call. (Finance's `financial-modeling-reviewer` declares `model: inherit` precisely because a wrong retirement number is expensive.) On Codex, where `model:` is Claude vocabulary, read the reviewer's frontmatter `codex_model:` (a literal Codex model ID) and pass it on `spawn_agent`; when the key is absent, pass no override.
...
**Project-custom reviewers — dispatch path.** Dispatch them the same way as a built-in persona, with one wrinkle: they live in the repo under review, not in this plugin. Where the harness auto-registers a project's `.claude/agents/` as subagent types (Claude Code does), dispatch by the agent's **bare** registered name — `financial-modeling-reviewer`, with no `engineering-loop:` plugin prefix — and pass the subagent template as the task prompt; the agent file is already the subagent's system prompt, so the persona need not be injected again. Where the harness has no project-agent registry, fall back to the portable path used for built-ins: read the agent file's body and inject it into the template's `{persona_file}` slot on a generic reviewer subagent. Codex has a project registry of its own at `.codex/agents/*.toml`, but the spawn tool cannot address a named agent from it, so Codex takes this portable path too; the model comes from `codex_model:` as above. Either way the reviewer receives the same review-context bundle (items 2-6 above) and is bound by the same read-only contract.
...
Each persona sub-agent writes full JSON (all schema fields) to `/tmp/engineering-loop/review/{run_id}/{reviewer_name}.json` and returns compact JSON with merge-tier fields only:
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - `why_it_matters` when available — loaded from the per-agent artifact file at `/tmp/engineering-loop/review/{run_id}/{reviewer_name}.json`; omit when the file is absent or the artifact write failed. The validator proceeds without it, using the diff and cited code directly.
   - The full diff
...
Artifact: /tmp/engineering-loop/review/<run-id>/
...
**Detail enrichment (headless only):** The headless envelope includes `Why:`, `Evidence:`, and `Suggested fix:` lines. After merge (Stage 5), read the per-agent artifact files from `/tmp/engineering-loop/review/{run_id}/` for only the findings that survived dedup and confidence gating.
...
- In interactive, autofix, and headless modes, write a per-run artifact under `/tmp/engineering-loop/review/<run-id>/` containing:
